"""Figures for biased testimony, adaptive trust, and robustness."""
...
def figure(results: str, out: str) -> None:
    endogenous_path = os.path.join(results, "adaptive_trust_under_bias.csv")
    grid_path = os.path.join(results, "fixed_testimony_grid.csv")
    robustness_path = os.path.join(results, "correlation_penalty_robustness.csv")
    trajectories_path = os.path.join(results, "adaptive_trust_trajectories.npz")
    ...
    if _need(grid_path):
        ...
        for topology in topologies:
            ...
            plot_curve(
                ...
                se=subset["se"] if "se" in subset.columns else None,
            )
        ...
        _save(fig, out, "network_structure_inversion.png")
    ...
    if _need(endogenous_path):
        ...
        for ax, topology in zip(axes, topologies):
            ...
            for adaptive, color, label, linestyle in [
                ...
                curve = (
                    ...
                    .groupby("oracle_r", as_index=False)
                    .agg(reliability=("reliability", "mean"), se=("se", "mean"))
                ...
                plot_curve(
                    ...
                    linestyle=linestyle,
                    label=label,
                    se=curve["se"] if "se" in curve.columns else None,
                )
        ...
        _save(fig, out, "adaptive_trust_under_bias.png")
    ...
    if _need(trajectories_path):
        ...
        _save(fig, out, "adaptive_trust_trajectories.png")
    ...
    if _need(robustness_path):
        ...
        fig, ax = plt.subplots(figsize=SINGLE)
        image, _, _, _ = smooth_heatmap(ax, x, y, z, cmap=DIVERGING_CMAP, norm=norm)
        ...
        _save(fig, out, "correlation_penalty_robustness.png")
